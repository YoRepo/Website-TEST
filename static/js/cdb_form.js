// path: static/js/cdb_form.js
// State-driven .cdb editor. A flat list of cards (no sections); each card
// carries the passcode/id it should get in the generated database. Uses the
// same visual card picker as the article editor. On submit the list is
// serialised into the hidden #structure field and the server streams back a
// downloadable .cdb.

document.addEventListener("DOMContentLoaded", () => {
  const root = document.getElementById("cdb-editor");
  if (!root) return;

  const form = document.getElementById("cdb-form");
  const hidden = document.getElementById("structure");
  const host = document.getElementById("cdb-cards");
  const addCardBtn = document.getElementById("cdb-addcard");
  ...
  let state;
  try { state = JSON.parse(document.getElementById("cdb-structure").textContent || "{}"); }
  catch { state = { cards: [] }; }
  ...
  function renderCard(c, ci) {
    ...
    const idInput = row.querySelector(".acard__cdbid");
    idInput.value = c.cdb_id || "";
    idInput.addEventListener("input", (e) => {
      c.cdb_id = e.target.value.replace(/[^0-9]/g, "");
      if (c.cdb_id !== e.target.value) e.target.value = c.cdb_id;
    });
